...
import { addWord as dbAddWord, updateWord as dbUpdateWord, deleteWord as dbDeleteWord } from './dbUtils';
...
const wordsLearningSlice = createSlice({
  ...
  reducers: {
    ...
    updateWord(state, action) {
      ...
      state.words[wordIndexToUpdate] = updatedWord;
      dbUpdateWord(updatedWord);
    },
    removeWord(state, action) {
      const wordToDelete = action.payload;
      state.words = state.words.filter(word => word.word !== wordToDelete);
      dbDeleteWord(wordToDelete);
    },
